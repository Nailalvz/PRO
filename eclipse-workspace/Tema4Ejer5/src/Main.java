...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (numeroDeEmpleados() > 0) {
            ...
            System.out.printf("Sueldo: %.2f € %n", e.SueldoBruto());
            ...
            System.out.println(e);
            System.out.printf("Total horas extras: %.2f €" , e.CalculoExtras());
    ...
    public static Empleado empleadoQueMasCobra() {
        ...
        for (int i = 1; i < indice; i++) {
            if (empleados[i].SueldoBruto() > mayor.SueldoBruto()) {
                mayor = empleados[i];
            }
        }
        return mayor;
    ...
    public static Empleado empleadoQueMenosCobra() {
        ...
        for (int i = 1; i < indice; i++) {
            if (empleados[i].SueldoBruto() < menor.SueldoBruto()) {
                menor = empleados[i];
            }
        }
        return menor;
    ...
    public static Empleado empleadoQueMasCobraPorHorasExtras() {
        Empleado mayor = empleados[0];
        for (int i = 1; i < indice; i++) {
            if (empleados[i].CalculoExtras() > mayor.CalculoExtras()) {
                mayor = empleados[i];
            }
        }
        return mayor;
    ...
    public static Empleado empleadoQueMenosCobraPorHorasExtras() {
        Empleado menor = empleados[0];
        for (int i = 1; i < indice; i++) {
            if (empleados[i].CalculoExtras() < menor.CalculoExtras()) {                 
                menor = empleados[i];
            }
        }
        return menor;
    ...
    public static void ordenarPorSalario() {
        ...
        for (i = 0; i < indice - 1; i++) {
            for (j = 0; j < indice - i - 1; j++) {
                if (empleados[j + 1].SueldoBruto() < empleados[j].SueldoBruto()) {
                    aux = empleados[j + 1];
    ...
    public static void mostrarEmpleadosYSalarios() {
        for (int i = 0; i < indice; i++) {
            System.out.println(empleados[i]);
            System.out.printf("Sueldo: %.2f € %n" , empleados[i].SueldoBruto());
        }
    }
